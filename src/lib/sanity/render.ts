...
export function renderPortableText(body: unknown): string {
	...
	const components = {
		block: {
			...
			heading: (opts: { children?: unknown; level?: number }) =>
				`<h${opts.level ?? 2}>${opts.children ?? ''}</h${opts.level ?? 2}>`,
			numberList: (opts: { children?: unknown }) => `<ol>${opts.children ?? ''}</ol>`,
			numberListItem: (opts: { children?: unknown }) => `<li>${opts.children ?? ''}</li>`,
			blockquote: (opts: { children?: unknown }) =>
				`<blockquote>${opts.children ?? ''}</blockquote>`,
		},
		marks: {
	...
	return toHTML(
		body as Parameters<typeof toHTML>[0],
		{ components } as Parameters<typeof toHTML>[1],
	)
}
